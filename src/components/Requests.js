...
export const requestUpdateReflectionDetail = (
  token,
  reflectionId,
  updateReflection
) => {
...
export const requestRecentReflectionDetail = (token, reflectionId) => {
  const url = `https://neoself-be-service.onrender.com/recent/reflection/${reflectionId}/`;
  const response = axios.get(url, {
    headers: { Authorization: `Token ${token}` },
  });
  return response;
